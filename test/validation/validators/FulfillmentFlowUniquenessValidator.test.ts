...
import { FulfillmentFlowUniquenessValidator } from '../../../src/validation/validators/FulfillmentFlowUniquenessValidator.js';

describe('FulfillmentFlowUniquenessValidator', () => {
  ...
  it('returns PASS skipped when no fulfillmentFlow', async () => {
    const result = await FulfillmentFlowUniquenessValidator.validate(ctx());
    expect(result.name).to.equal('FulfillmentFlowUniquenessValidator');
  ...
  it('returns PASS skipped when deploymentIntent is link', async () => {
    const result = await FulfillmentFlowUniquenessValidator.validate(
      ctx({
        fulfillmentFlow: {
          apiName: 'FulfillFlow',
          namespace: null,
