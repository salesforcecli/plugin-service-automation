...
import { IntakeFlowExistenceValidator } from '../../../src/validation/validators/IntakeFlowExistenceValidator.js';

describe('IntakeFlowExistenceValidator', () => {
  ...
  it('returns PASS skipped when no intakeFlow', async () => {
    const result = await IntakeFlowExistenceValidator.validate(ctx());
    expect(result.name).to.equal('IntakeFlowExistenceValidator');
  ...
  it('returns PASS skipped when deploymentIntent is deploy', async () => {
    const result = await IntakeFlowExistenceValidator.validate(
      ctx({
        intakeFlow: {
          apiName: 'MyFlow',
          namespace: null,
